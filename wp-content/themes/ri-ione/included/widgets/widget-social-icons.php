    ...
    class RITSocialWidget extends WP_Widget
    {
        public $socials = array(
            ...
            'ion-social-youtube' => array(
                ...
                'link' => 'http://www.youtube.com/chanel/*',
                'icon'=>'fa-youtube',
            ),
        ...
        {
            $widget_ops = array('classname' => 'RITSocialWidget', 'description' => esc_html__('Displays your social profile.', 'ri-ione'));

            parent::__construct(false, esc_html__('RIT Social', 'ri-ione'), $widget_ops);
        }
        ...
        function widget($args, $instance)
        {
            ...
            echo '<ul class="rit-widget-social-icon primary-font '.esc_attr($instance['mode']).' clearfix">';
        ...
        function form($instance)
        {
            ...
            ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php echo esc_html__('Title', 'ri-ione'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('mode')); ?>"><?php echo esc_html__('Display', 'ri-ione'); ?></label>
                <select id="<?php echo esc_attr($this->get_field_id('mode')); ?>" name="<?php echo esc_attr($this->get_field_name('mode')); ?>" style="width:100%;">
                    <option value='text' <?php if ('text' == $instance['mode'] ||  $instance['mode']=='') echo 'selected="selected"'; ?>><?php echo esc_html__('Only Text', 'ri-ione'); ?></option>
                    <option value='icon' <?php if ('icon' == $instance['mode']) echo 'selected="selected"'; ?>><?php echo esc_html__('Only Icon', 'ri-ione'); ?></option>
                    <option value='both' <?php if ('both' == $instance['mode']) echo 'selected="selected"'; ?>><?php echo esc_html__('Both Text and Icon', 'ri-ione'); ?></option>
                </select>
            </p> <?php
            foreach ($this->socials as $key => $social) {
                ?>
