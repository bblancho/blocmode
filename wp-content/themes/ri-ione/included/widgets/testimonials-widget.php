...
class rit_testimonials_widget extends WP_Widget {
    ...
    function __construct() {
        /* Widget settings. */
        $widget_ops = array( 'classname' => 'rit_testimonials_widget', 'description' => esc_html__('A widget display testimonials follow category', 'ri-ione') );

        /* Widget control settings. */
        $control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rit_testimonials_widget' );

        /* Create the widget. */
        parent::__construct( 'rit_testimonials_widget', esc_html__('RIT: Testimonials', 'ri-ione'), $widget_ops, $control_ops );
    }
...
<?php
    ...
    function form( $instance ) {
        ...
        $defaults = array( 'title' => esc_html__('Testimonials', 'ri-ione'), 'number' => 3, 'categories' => '','type'=>'grid','pre_style'=>'default','order_by'=>'date','columns'=>1,'output_type'=> 'excerpt');
        $instance = wp_parse_args( (array) $instance, $defaults ); ?>
        <!-- Widget Title: Text Input -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html(esc_html_e('Title:', 'ri-ione')); ?></label>
            <input  type="text" class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($instance['title']); ?>"  />
        </p>

        <!-- Category -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('categories')); ?>"><?php echo esc_html(__('Filter by Category:', 'ri-ione')); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories" style="width:100%;">
                <option value='' <?php if ('' == $instance['categories']) echo 'selected="selected"'; ?>><?php echo esc_html(__('All categories', 'ri-ione')); ?></option>
        ...
        <!-- Number of posts -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e('Number of posts to show:', 'ri-ione'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'type' )); ?>"><?php esc_html_e('Type display:', 'ri-ione'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('type')); ?>" name="<?php echo esc_attr($this->get_field_name('type')); ?>" class="widefat style" style="width:100%;">
                <option value='grid' <?php if ('grid' == $instance['type']||$instance['type']=='') echo 'selected="selected"'; ?>><?php echo esc_html__('Grid', 'ri-ione'); ?></option>
                <option value='carousel' <?php if ('carousel' == $instance['type']) echo 'selected="selected"'; ?>><?php echo esc_html__('Carousel', 'ri-ione'); ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'pre_style' )); ?>"><?php esc_html_e('Style display:', 'ri-ione'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('pre_style')); ?>" name="<?php echo esc_attr($this->get_field_name('pre_style')); ?>" class="widefat style" style="width:100%;">
                <option value='default' <?php if ('default' == $instance['pre_style']) echo 'selected="selected"'; ?>><?php echo esc_html__('Default', 'ri-ione'); ?></option>
                <option value='style-1' <?php if ('style-1' == $instance['pre_style']) echo 'selected="selected"'; ?>><?php echo esc_html__('Style 1', 'ri-ione'); ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'order_by' )); ?>"><?php esc_html_e('Order by:', 'ri-ione'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('order_by')); ?>" name="<?php echo esc_attr($this->get_field_name('order_by')); ?>" class="widefat style" style="width:100%;">
                <option value='date' <?php if ('date' == $instance['order_by']) echo 'selected="selected"'; ?>><?php echo esc_html__('Latest', 'ri-ione'); ?></option>
                <option value='random' <?php if ('random' == $instance['order_by']) echo 'selected="selected"'; ?>><?php echo esc_html__('Random', 'ri-ione'); ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'columns' )); ?>"><?php esc_html_e('Columns:', 'ri-ione'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('columns')); ?>" name="<?php echo esc_attr($this->get_field_name('columns')); ?>" class="widefat style" style="width:100%;">
                <option value='1' <?php if ('1' == $instance['columns']) echo 'selected="selected"'; ?>><?php echo esc_html__('1', 'ri-ione'); ?></option>
                <option value='2' <?php if ('2' == $instance['columns']) echo 'selected="selected"'; ?>><?php echo esc_html__('2', 'ri-ione'); ?></option>
                <option value='3' <?php if ('3' == $instance['columns']) echo 'selected="selected"'; ?>><?php echo esc_html__('3', 'ri-ione'); ?></option>
                <option value='4' <?php if ('4' == $instance['columns']) echo 'selected="selected"'; ?>><?php echo esc_html__('4', 'ri-ione'); ?></option>
                <option value='5' <?php if ('5' == $instance['columns']) echo 'selected="selected"'; ?>><?php echo esc_html__('5', 'ri-ione'); ?></option>
                <option value='6' <?php if ('6' == $instance['columns']) echo 'selected="selected"'; ?>><?php echo esc_html__('6', 'ri-ione'); ?></option>
            </select>
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'output_type' )); ?>"><?php esc_html_e('Content:', 'ri-ione'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('output_type')); ?>" name="<?php echo esc_attr($this->get_field_name('output_type')); ?>" class="widefat style" style="width:100%;">
                <option value='excerpt' <?php if ('excerpt' == $instance['output_type']) echo 'selected="selected"'; ?>><?php echo esc_html__('Excerpt', 'ri-ione'); ?></option>
                <option value='full' <?php if ('full' == $instance['output_type']) echo 'selected="selected"'; ?>><?php echo esc_html__('Full content', 'ri-ione'); ?></option>
            </select>
        </p>
