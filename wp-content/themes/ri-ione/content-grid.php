...
$rit_columns = get_theme_mod('rit_default_post_col', 1);
switch ($rit_columns) {
    case '2':
...
?>
<article <?php echo post_class($rit_class) ?>>
    ...
    <div class="rit-post-inner">
        ...
        <?php } ?>
            <?php the_title(sprintf('<h3 class="entry-title title-post"><a href="%s" rel="' . esc_html__('bookmark', 'ri-ione') . '">', esc_url(get_permalink())), '</a></h5>'); ?>
        </div>
        ...
        if (get_theme_mod('rit_hide_readmore') != '1') {
            ?>
            <p class="readmore">
                <a href="<?php echo esc_url(the_permalink()); ?>"><?php echo esc_html__('Continue reading', 'ri-ione'); ?></a>
            </p>
        <?php } ?>
    </div>
</article>