...
if( !function_exists('vc_field_animation_type')) {
    function vc_field_animation_type($settings, $value) {
        ...
        $param_line .= '</optgroup>';
        $param_line .= '<optgroup label="' . esc_html__('Zoom Entrances', 'rit-core-language') . '">';
...
function vc_field_image_radio($settings, $value) {
    ...
    if(count($settings['value']) > 0 ){
        foreach($settings['value'] as $param => $param_val) {
            ...
            $output .= '<img class="rit-image-radio-'.esc_attr($settings['param_name']).'" src="'.esc_url($param).'" data-value="'.esc_attr($param_val).'" style="width:'.esc_attr($width).';border-style: solid;border-width: 5px;border-color: '.esc_attr($border_color).';margin-left: 15px;">';
        }
        $output .= '<script type="text/javascript">
...
function vc_field_datepicker($settings, $value) {
    $type = isset($settings['type']) ? $settings['type'] : '';
    $class = isset($settings['class']) ? $settings['class'] : '';
    $output = '<input class="wpb_vc_param_value '. esc_attr($settings['param_name']).' '.esc_attr($type).' '.esc_attr($class).'"  type="date" name="'.esc_attr($settings['param_name']).'" value="'.esc_attr($value).'">';
    $output.=  '<script>
    jQuery( function() {
        jQuery( ".rit_datepicker" ).datepicker();
    } );
  </script>';
    return $output;
}
...
if (!function_exists('vc_add_shortcode_param')){
    ...
    vc_add_shortcode_param('rit_datepicker', 'vc_field_datepicker');
